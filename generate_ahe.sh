...
vrt_file=generator_"$moment"_"$mon".vrt
gdalbuildvrt "$vrt_file" -tr 0.0083333333333333 0.0083333333333333 -te -180. -90. 180. 90. \
...
sed -i '/<VRTRasterBand/{
  ...
  a <PixelFunctionType>ahe.monthly_no_meta</PixelFunctionType>
}' "$vrt_file"
gdal_translate -ot Float32 -co COMPRESS=LZW -co PREDICTOR=2 "$vrt_file" ./"$moment"/AHE_no_meta_SSP3_"$year"_"$mon".tif
rm "$vrt_file"

vrt_base_file=generator_"$moment"_"$mon".vrt
gdalbuildvrt "$vrt_base_file" -tr 0.0083333333333333 0.0083333333333333 -te -180. -90. 180. 90. \
  ./"$moment"/AHE_no_meta_SSP3_"$year"_"$mon".tif ./monthly_temp/"$moment"/"$mon"_tempmask.tif "$popden"
for hor in {0..23}; do
  (
    horf=$(printf '%02d' "$hor")
    vrt_file=generator_"$moment"_"$mon"_"$horf"HR.vrt
    sed '/<VRTRasterBand/{
      s/<VRTRasterBand/& subClass="VRTDerivedRasterBand"/
      a <PixelFunctionLanguage>Python</PixelFunctionLanguage>
      a <PixelFunctionType>ahe.hourly</PixelFunctionType>
      '"$(printf 'a <PixelFunctionArguments w1="%lf" w2="%lf" w3="%lf" w4="%lf" fact="%lf"/>' "${w1[$hor]}" "${w2[$hor]}" "${w3[$hor]}" "${w4[$hor]}" "${fact[$hor]}")"'
    }' "$vrt_base_file" > "$vrt_file"
    temp_tif=temp_"$moment"_"$mon"_"$horf"HR.tif
    gdal_translate -ot Float32 -co COMPRESS=LZW -co PREDICTOR=2 "$vrt_file" "$temp_tif"
    gdal_translate -ot Int32 -co COMPRESS=LZW -co PREDICTOR=2 "$temp_tif" ./"$moment"/AHE_SSP3_"$year"_"$mon"_"$horf"HR.tif
    rm "$temp_tif" "$vrt_file"
  ) &
done
wait
rm "$vrt_base_file"
...
for horf in {00..23}; do
  (
    vrt_file=generator_"$moment"_utc_"$mon"_"$horf".vrt
    gdalbuildvrt "$vrt_file" -tr 0.0083333333333333 0.0083333333333333 -te -180. -90. 180. 90. \
      ./"$moment"/AHE_SSP3_"$year"_"$mon"_{00..23}HR.tif ./timezone/time_"$horf".tif
    sed -i '/<VRTRasterBand/{
      s/<VRTRasterBand/& subClass="VRTDerivedRasterBand"/
      a <PixelFunctionLanguage>Python</PixelFunctionLanguage>
      a <PixelFunctionType>ahe.utc</PixelFunctionType>
    }' "$vrt_file"
    gdal_translate -ot Int32 -co COMPRESS=LZW -co PREDICTOR=2 "$vrt_file" ./"$moment"_utc/AHE_SSP3_"$year"_"$mon"_"$horf"HR_UTC.tif
    rm "$vrt_file"
  ) &
done
wait
